...
/// Object that represents one running processes to which probes can be attached.
...
mod tests {
    ...
    fn process_create() -> Result<()> {
        ...
        // -1 should yield a pid higher than typical PID_MAX.
    ...
    fn shared_libs() -> Result<()> {
        let p = Process::from_pid(std::process::id() as i32)?;
        // Linux's ld has, itself a USDT probe, check if we can probe it.
        let traceable = p.is_usdt("rtld::map_start");
        assert!(traceable.is_ok() && traceable.unwrap());
        Ok(())
    }
}
