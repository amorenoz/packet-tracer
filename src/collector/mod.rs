...
trait Collector {
    ...
    fn init(&mut self, probes: &mut Probes) -> Result<()>;
    /// Start the collection of events (non-probes).
    fn start(&mut self) -> Result<()>;
}

/// Collections of collectors. Used to handle a set of collectors and to perform
/// group actions.
pub(crate) struct CollectorsCollection<'a> {
    list: HashMap<String, Box<dyn Collector>>,
    probes: Probes<'a>,
}

impl<'a> CollectorsCollection<'a> {
    fn new() -> Result<CollectorsCollection<'a>> {
        Ok(CollectorsCollection {
            list: HashMap::new(),
            probes: Probes::new()?,
        })
    }
    ...
    pub(crate) fn init(&mut self) -> Result<()> {
        ...
        for (_, c) in self.list.iter_mut() {
            if c.init(&mut self.probes).is_err() {
...
pub(crate) fn get_collectors<'a>() -> Result<CollectorsCollection<'a>> {
